...
import { Search, CheckCircle2, ArrowRight, ChevronRight } from 'lucide-react';
...
function PhoneFrame({ children, shadowColor = 'shadow-gray-200' }: { children: React.ReactNode; shadowColor?: string }) {
  return (
    <div className={`relative w-56 rounded-[2.2rem] bg-gray-900 border-[5px] border-gray-800 shadow-2xl ${shadowColor} overflow-hidden`}>
...
function SearchPhone() {
  return (
    <PhoneFrame shadowColor="shadow-blue-100">
      <div className="bg-blue-600 px-3 py-2.5 flex items-center gap-2">
        <div className="h-2 w-14 bg-blue-300 rounded" />
        <div className="ml-auto w-5 h-5 bg-blue-500 rounded-full" />
      </div>
      ...
      <div className="mx-2.5 mt-1.5 flex gap-1">
        <div className="bg-blue-50 border border-blue-100 rounded-full px-2 py-0.5 flex items-center gap-1">
          <div className="w-1.5 h-1.5 bg-blue-500 rounded-full" />
          <div className="h-1.5 w-10 bg-blue-300 rounded" />
        </div>
      ...
      <div className="mx-2.5 mt-1.5 grid grid-cols-3 gap-1.5">
        {[['bg-blue-100','bg-blue-500'],['bg-yellow-100','bg-yellow-500'],['bg-emerald-100','bg-emerald-500'],
      ...
      <div className="mx-2.5 mb-3 space-y-1.5">
        {[true, false].map((active, i) => (
          <div key={i} className={`bg-white rounded-xl p-2 border ${active ? 'border-blue-300 shadow-sm' : 'border-gray-100'} flex items-center gap-2`}>
            <div className={`w-8 h-8 rounded-full flex-shrink-0 ${active ? 'bg-blue-200' : 'bg-gray-200'}`} />
            ...
            <div className="h-5 w-10 bg-blue-100 rounded-lg" />
          </div>
        ))}
      </div>
    </PhoneFrame>
...
function TrackingPhone() {
  return (
    <PhoneFrame shadowColor="shadow-sky-100">
      ...
      {/* Map area */}
      <div className="mx-2.5 h-32 bg-gradient-to-br from-sky-50 to-blue-100 rounded-xl relative overflow-hidden border border-sky-100">
        ...
        <div className="absolute top-[52%] left-[58%] flex flex-col items-center">
          ...
          <div className="w-1 h-2 bg-blue-600" />
        </div>
      </div>
      {/* Status bar */}
      ...
      <div className="px-3 mb-3">
        ...
        <div className="grid grid-cols-7 gap-0.5">
          {[...Array(28)].map((_, i) => {
            ...
            const isToday = i === 4;
            return (
              <div key={i} className={`h-5 rounded flex items-center justify-center ${isToday ? 'bg-blue-600' : isOn ? 'bg-blue-100' : 'bg-gray-50'}`}>
                <div className={`w-1.5 h-1.5 rounded-full ${isToday ? 'bg-white' : isOn ? 'bg-blue-400' : 'bg-gray-200'}`} />
              </div>
            );
          })}
        </div>
      ...
      <div className="px-3 border-t border-gray-100 pt-2 pb-3">
        ...
        {[true, false].map((on, i) => (
          <div key={i} className="flex items-center justify-between mb-1.5">
            ...
            <div className={`w-8 h-4 rounded-full ${on ? 'bg-blue-500' : 'bg-gray-200'}`} />
          </div>
        ))}
      </div>
    </PhoneFrame>
...
function EarningsPhone() {
  return (
    <PhoneFrame shadowColor="shadow-amber-100">
      <div className="bg-gradient-to-r from-indigo-600 to-blue-500 px-4 pt-3 pb-6">
      ...
      <div className="-mt-3 mx-2.5 bg-white rounded-xl border border-gray-100 shadow-sm p-2.5 mb-2">
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {[['bg-blue-50','bg-blue-400'],['bg-emerald-50','bg-emerald-400'],['bg-amber-50','bg-amber-400']].map(([bg, dot], i) => (
...
// ─── Step wrapper: alternating left/right ────────────────────────────────────
function Step({
  stepLabel, badgeColor, heading, description, subSteps, checkColor,
  illustration, flip = false,
}: {
  stepLabel: string; badgeColor: string; heading: React.ReactNode; description: string;
  ...
  return (
    <div className="grid md:grid-cols-2 gap-12 lg:gap-20 items-center">
      ...
      <div className={flip ? 'order-1 md:order-2' : ''}>
        <div className={`inline-flex items-center ${badgeColor} text-white text-xs font-black px-3 py-1 rounded-full mb-5`}>
          {stepLabel}
        </div>
        <h2 className="text-2xl md:text-3xl font-extrabold text-gray-900 mb-4 leading-tight">{heading}</h2>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="bg-white border-b border-gray-100 pt-20 pb-16">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <span className="inline-block bg-blue-50 text-blue-700 text-xs font-black px-4 py-1.5 rounded-full uppercase tracking-widest mb-6 border border-blue-100">
            Simple Process
          </span>
          ...
          <div className="inline-flex bg-gray-100 rounded-2xl p-1.5 gap-1">
            <button
              ...
              className={`px-6 py-2.5 rounded-xl font-bold text-sm transition-all ${tab === 'client' ? 'bg-white text-blue-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
            >
              I need a service
            </button>
            <button
              onClick={() => setTab('helper')}
              className={`px-6 py-2.5 rounded-xl font-bold text-sm transition-all ${tab === 'helper' ? 'bg-white text-indigo-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
            >
              I want to earn
            </button>
          </div>
      ...
      {tab === 'client' && (
        <section className="py-20">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-20">
            ...
            <Step
              stepLabel="STEP 01" badgeColor="bg-blue-600" checkColor="text-blue-500"
              heading={<>Search &amp; discover<br />the right professional</>}
              description="Browse over 20 service categories or type exactly what you need — plumber, electrician, cleaner and more. Filter by city, budget, and rating to find your perfect match in seconds."
            ...
            <Connector from="from-blue-300 to-emerald-300" to="text-emerald-400" />

            <Step
              stepLabel="STEP 02" badgeColor="bg-emerald-600" checkColor="text-emerald-500"
            ...
            <Step
              stepLabel="STEP 03" badgeColor="bg-sky-500" checkColor="text-sky-500"
            ...
            <Step
              stepLabel="STEP 04" badgeColor="bg-violet-600" checkColor="text-violet-500"
            ...
            <Step
              stepLabel="STEP 05" badgeColor="bg-amber-500" checkColor="text-amber-500"
          ...
          <div className="mt-20 text-center px-4">
            <div className="inline-flex flex-col sm:flex-row items-center gap-5 bg-white border border-gray-200 rounded-2xl px-8 py-5 shadow-sm">
              ...
              <Link href="/#search" className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-sm transition-all shadow-md shadow-blue-200 hover:-translate-y-0.5 flex items-center gap-1.5 whitespace-nowrap">
                <Search size={14} />
                Book Now
              </Link>
            </div>
      ...
      {tab === 'helper' && (
        <section className="py-20">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-20">
            ...
            <Step
              stepLabel="STEP 01" badgeColor="bg-indigo-600" checkColor="text-indigo-500"
            ...
            <Connector from="from-indigo-300 to-blue-300" to="text-blue-400" />

            <Step
              stepLabel="STEP 02" badgeColor="bg-blue-600" checkColor="text-blue-500"
            ...
            <Step
              stepLabel="STEP 03" badgeColor="bg-emerald-600" checkColor="text-emerald-500"
            ...
            <Step
              stepLabel="STEP 04" badgeColor="bg-amber-500" checkColor="text-amber-500"
